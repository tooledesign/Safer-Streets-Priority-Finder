...
if (!is.null(params$year_table)){
  crashes_by_year <-  plotly::plot_ly(params$year_table, x = ~Year, y = ~`Fatality (K)`, name = 'Fatality (K)', type = 'bar')  %>%
    add_trace(y = ~`Incapacitating Injury (A)`, name = 'Incapacitating Injury (A)')  %>%
    add_trace(y = ~`Non-Incapacitating Injury (B)`, name = 'Non-Incapacitating Injury (B)')  %>%
    add_trace(y = ~`Possible Injury (C)`, name = 'Possible Injury (C)')  %>%
    add_trace(y = ~`Property Damage Only (O)`, name = 'Property Damage Only (O)')  %>%
